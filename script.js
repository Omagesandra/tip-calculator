...
btns.forEach((btn) => {
    btn.addEventListener('click', (e) => {
        ...
        calculatorTip(numberOfPeople, percent, amount)
    } )
})
...
// CALCULATOR TIP
const calculatorTip = (numberOfPeople, percent, amount) => {
    if (!numberOfPeople || !amount || !percent) return
    const tipPerPerson = ((percent / 100) * amount).toFixed(2)
    const sumOfTip = (numberOfPeople * tipPerPerson).toFixed(2)
    tip.textContent = `$${tipPerPerson}`
    totalTip.textContext = `$${sumOfTip}`
}
